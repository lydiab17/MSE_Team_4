...
public class VotingFxController {
  ...
  @FXML private ListView<VotingResponse> notOpenVotingsList;

  @FXML private Label statusLabel;

  @FXML private TableView<OptionResultResponse> resultsTable;

  @FXML private TableColumn<OptionResultResponse, String> optionColumn;

  @FXML private TableColumn<OptionResultResponse, Long> countColumn;
  ...
  private void initialize() {
    ...
    notOpenVotingsList.setCellFactory(
        lv ->
            new ListCell<>() {
              @Override
              protected void updateItem(VotingResponse item, boolean empty) {
    ...
    optionColumn.setCellValueFactory(cell -> new ReadOnlyStringWrapper(cell.getValue().option()));
  ...
  private void onCreateVoting() {
    ...
    try {
      ...
      CreateVotingRequest req =
          new CreateVotingRequest(id, name, info, start, end, options.stream().toList());
  ...
  private String formatVoting(VotingResponse v) {
    return String.format(
        "ID %d - %s [%s bis %s]%s",
        v.id(), v.name(), v.startDate(), v.endDate(), v.open() ? " (offen)" : "");
  }
  ...
  private <T> void runAsync(
      ThrowingSupplier<T> work, UiConsumer<T> onSuccess, UiConsumer<Throwable> onError) {
    Task<T> task =
        new Task<>() {
          @Override
  ...
  private record Lists(
      java.util.List<VotingResponse> open, java.util.List<VotingResponse> notOpen) {}
