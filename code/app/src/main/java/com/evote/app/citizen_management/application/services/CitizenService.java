 ...
 *
 * <p>Der {@code CitizenService} implementiert die fachlichen Anwendungsfälle rund um Bürger (z. B.
 * Registrierung, Login, Abfrage des aktuell eingeloggten Bürgers). Er koordiniert dabei die
 * Zusammenarbeit zwischen Aggregator, Projector und Repositories.
 *
 * <p>Der Service enthält selbst keine fachlichen Entscheidungen, sondern steuert den Ablauf der
 * Use-Cases und delegiert Entscheidungen an den {@link CitizenAggregator} sowie die technische
 * Umsetzung an den {@link CitizenProjector}.
 */
@Service
public class CitizenService {
   ...
   *
   * <p>Die vom Client übergebenen Registrierungsdaten werden zunächst in einen {@link
   * CitizenRegistrationCommand} umgewandelt. Dieser Command wird an den {@link CitizenAggregator}
   * delegiert, der die fachliche Entscheidung trifft und entsprechende Domain Events erzeugt.
   *
   * <p>Das erzeugte {@link CitizenCreatedEvent} wird anschließend durch den {@link
   * CitizenProjector} in einen konkreten Citizen-Zustand projiziert und persistiert.
   *
   ...
   *
   * <p>Überprüft, ob ein Bürger mit der angegebenen E-Mail existiert und ob das übergebene Passwort
  ...
  public boolean loginCitizen(String email, String passwort) {
    ...
    // falls Citizen existiert: Passwort aus Domain-Objekt holen und mit übergebenem Passwort
    // vergleichen
   ...
   *
   * <p>Die E-Mail-Adresse des aktuell authentifizierten Benutzers wird aus dem Spring Security
   * Context gelesen. Anschließend werden die zugehörigen Bürgerdaten aus dem {@link
   * CitizenRepository} geladen und als {@link CitizenDto} zurückgegeben.
   *
