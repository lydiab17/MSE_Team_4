 ...
 *
 * <p>Der Aggregator ist für die Verarbeitung von Commands zuständig und stellt
 * sicher, dass alle fachlichen Regeln (Invarianten) eingehalten werden.
 * Er entscheidet, ob ein Command ausgeführt werden darf oder abgelehnt wird.</p>
 *
 * <p>Bei erfolgreicher Verarbeitung eines Commands erzeugt der Aggregator
 * entsprechende {@link DomainEvent}s und speichert diese im {@link EventStore}.
 * Der Aggregator selbst verändert dabei keinen persistierten Zustand direkt.</p>
 */
@Service
public class CitizenAggregator {
     ...
     *
     * <p>Der Aggregator prüft zunächst, ob der Command fachlich zulässig ist
     * (z. B. ob die E-Mail-Adresse bereits existiert). Ist der Command gültig,
     * wird ein {@link CitizenCreatedEvent} erzeugt und im {@link EventStore}
     * gespeichert.</p>
     *
