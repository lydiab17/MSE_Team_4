...
public class VoteCastingService {
   ...
   * <ol>
   *   <li>Voting laden
   *   <li>prüfen, ob Voting geöffnet ist
   *   <li>prüfen, ob Option existiert
   *   <li>prüfen, ob dieser "Wähler" schon abgestimmt hat
   *   <li>Vote erzeugen und speichern
   * </ol>
   *
   * <p>Hinweis: {@code dto.authToken()} wird hier vorläufig als einfacher voterKey verwendet.
   * Später wird das durch ein echtes Pseudonym/Token aus dem citizen_management ersetzt.
   *
  ...
  public void castVote(CastVoteDto dto) {
    ...
    boolean optionExists =
        voting.getOptionTexts().stream().anyMatch(o -> o.equalsIgnoreCase(dto.optionId()));
    if (!optionExists) {
